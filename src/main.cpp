...
class VMRApp { //this will store Vulkan objects (as private class members) and also functions initiating them 
public:
    void run() {
        initWindow();
        initVulkan();
...
private:
   ...
   void initVulkan() {
    ...
    void createInstance() {
        ...
        appInfo.pApplicationName = "Vulkan Material Editor";
        ...
        //which global (applied to entire program and not a specific device) extensions and validation layers we want to use
    ...
    void createLogicalDevice() {
        ...
        if (enableValidationLayers) { 
            //these 2 are ignored in newer implementations, because instance and device specific validation layers are no longer distinguished
            createInfo.enabledLayerCount = static_cast<uint32_t>(validationLayers.size());
            createInfo.ppEnabledLayerNames = validationLayers.data();
        } else {
            createInfo.enabledLayerCount = 0;
    ...
    static void framebufferResizeCallback(GLFWwindow* window, int width, int height) {
        auto app = reinterpret_cast<VMRApp*>(glfwGetWindowUserPointer(window));
